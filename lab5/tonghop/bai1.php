...
abstract class Hinh{
	...
	public function getDodai($doDai){
		return $this->dodai;
	}
...
class HinhTamGiacThuong extends Hinh{
	public function tinh_CV(){
		return array_sum($this->dodai);//$this->dodai[0]+$this->dodai[1]+$this->dodai[2];
	}
	public function tinh_DT(){
...
if(isset($_POST['tinh'])){
	...
	if(isset($_POST['hinh']) && ($_POST['hinh'])=="htgt"){
		...
		$htgt->setDodai($arr);
		$str= "Diện tích của hình tròn ".$htgt->getTen()." là : ".$htgt->tinh_DT()." \n".
				"Chu vi của hình tròn ".$htgt->getTen()." là : ".$htgt->tinh_CV();
	}
		...
		<tr>
			...
			<td><input type="radio" name="hinh" value="hv"
				...
				<input type="radio" name="hinh" value="hcn"
					<?php if(isset($_POST['hinh'])&&($_POST['hinh'])=="hcn") echo 'checked'?>/>Hình tam chữ nhật
			</td>
		</tr>
		<tr>
			<td>Nhập tên:</td>
